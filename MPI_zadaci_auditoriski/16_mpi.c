...
/*
  Compile: mpicc -O -o 16_mpi 16_mpi.c
  Execute: mpirun -np 4 16_mpi
*/
...
int main(int argc, char *argv[]) {
	...
	MPI_Reduce(&local_result, &global_result, 1, MPI_INT, MPI_SUM, source, MPI_COMM_WORLD);

	if (id == source) {
		printf("Brojot na elementi shto ispolnuvaat e %d.\n", global_result);
	}
